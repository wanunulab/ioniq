...
class Parser(object):
    ...
    def to_dict(self):
        d = {key: val for key, val in list(self.__dict__.items()) if key != 'param_dict' if type(val)
             in (int, float) or ('Qt' not in repr(val)) and 'lambda' not in repr(val)}
...
class SpikeParser(Parser):
    ...
    # TODO: Add super()__init__ to isinitate class PArser?
            ...
            if type(self._prominence_snr) in [tuple, list]:
                try:
                    ...
                    self.prominence = (self._prominence_snr[0] * level if self._prominence_snr[0] is not None else None,
                                       self._prominence_snr[1] * np.abs(mean_1stQ) if self._prominence_snr[1] is not None
                                       else self.prominence[1] if type(self.prominence) in [list, tuple] else None)
...
class lambda_event_parser(Parser):  # TODO: Rename class to Upper case
    """
    ...
    def __init__(self, threshold=90, rules=None):
        super().__init__()  # TODO: Add super()__init__ to isinitate class PArser?
...
class SpeedyStatSplit(Parser):
    ...
    uiTemplate = [("sampling_freq", "spin",
                   {'value': 200000, 'step': 1, 'dec': True, 'bounds': [0, None], 'suffix': 'Hz', 'siPrefix': True}),
...
class snakebase_parser(Parser):  # TODO: Upper case for class namr
    """
...
class FilterDerivativeSegmenter(Parser):
    ...
    def parse(self, current):
        ...
        for start, end in zip(tics[:-1:2], tics[1::2]):  # For all pairs of edges for a block
            ...
            if np.argmax(segment) > self.high_threshold:  # If the max derivative in that block is above a threshold
        ...
        return [Segment(current=current[tics[i]:tics[i + 1]], start=tics[i]) for i in range(0, len(tics) - 1, 2)]
