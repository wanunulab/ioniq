...
def detect_cusum(data, basesd, dt, threshhold=10, stepsize=3,
	...
	while k < length - 1:
		...
		if (
				variance == 0):  # with low-precision data sets it is possible that two adjacent values are equal, in which case there is zero variance for the two-vector of sample if this occurs next to a detected jump. This is very, very rare, but it does happen.
			variance = basesd * basesd  # in that case, we default to the local baseline variance, which is a good an estimate as any.
			print('entered')
		logp = stepsize * basesd / variance * (data[
												   k] - mean - stepsize * basesd / 2.)  # instantaneous log-likelihood for current sample assuming local baseline has jumped in the positive direction
		logn = -stepsize * basesd / variance * (data[
													k] - mean + stepsize * basesd / 2.)  # instantaneous log-likelihood for current sample assuming local baseline has jumped in the negative direction
	...
	cusum = dict()
	cusum['CurrentLevels'] = [np.average(data[int(edges[i] + minlength):int(edges[i + 1])]) for i in
